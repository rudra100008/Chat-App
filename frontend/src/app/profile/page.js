...
export default function ProfilePage() {
  ...
  const handleSave = (e) => {
    ...
    const handleUpdateUserData = async () => {
      try {
        const data = await updateUserDataService(userId, editProfile);
        setProfile(data);
        setShowEditButton(true);
      } catch (err) {
        console.error("Error in handleUpdateUserData: ", err.response?.data);
      }
    };
  ...
  const handleImageChange = async (event) => {
    ...
    try {
      const data = await updateUserImageService(userId, file);
      console.log("Image updated");
      setImageUpdateKey((prev) => prev + 1);
      await fetchCurrentUser();
    } catch (err) {
      console.log("Error in handleFileChange: ", err.response?.data);
    }
  };
              ...
              <div className={styles.avatarHeader}>
                <div className={styles.avatarRing}>
                  <GetUserImage
                    userId={userId}
                    size={58}
                    key={`left-${userId}-${imageUpdateKey}`}
                  />
                </div>
          ...
          <div className={styles.rightCol}>
            {activePanel === "profile" && (
              <div className={styles.editCard}>
                ...
                <div className={styles.editAvatarHeader}>
                  <div className={styles.editAvatarRing}>
                    <GetUserImage
                      userId={userId}
                      size={58}
                      key={`right-${userId}-${imageUpdateKey}`}
                    />
                    <span
                  ...
                  {showEditButton ? (
                    ...
                    <>
                      <input
                        ...
                        placeholder={profile.username || "N/A"}
                        onChange={(e) =>
                ...
                {/*About Yourself */}
                <div className={styles.fieldRow}>
                  <span className={`${styles.fieldLabel} ${styles.aboutLabel}`}>
                    About Yourself
                  </span>
                  {showEditButton ? (
                    <>
                    ...
                    <>
                      <textarea
                        className={`${styles.fieldValueEditable} ${styles.aboutTextarea}`}
                        placeholder={profile.about || "Enter about yourself"}
