...
//    }
    ...
    @GetMapping(path = "/{userId}/fetchImageUrl")
    public ResponseEntity<Map<String,String>> fetchUserSecureUrl(
            @PathVariable("userId")String userId
    )
    {
        try {
            String secureUrl = this.userService.getImageSecureUrlInCloud(userId);
            ...
            throw new ImageInvalidException(String.format("Failed to fetch image secure url: %S",e.getMessage()));
        }
    }
...
// this rest api updates image in file
...
//        }
//    }
    // this rest api updates user images in cloud
    @PatchMapping(path = "/{userId}/updateImage",consumes = {MediaType.MULTIPART_FORM_DATA_VALUE})
    public ResponseEntity<?> updateUserImageInCloud(
            @PathVariable("userId")String userId,
            @RequestParam("imageFile") MultipartFile imageFile
    )
    {
        try{
            UserDTO userDTO = this.userService.updateUserImageInCloud(userId,imageFile);
            return ResponseEntity.status(HttpStatus.OK).body(userDTO);
        }catch(IOException e){
            throw new ImageInvalidException(String.format("Image update failed: %s",e.getMessage()));
        }
    }
